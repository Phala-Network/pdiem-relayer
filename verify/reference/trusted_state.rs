...
impl TrustedState {
    ...
    /// FullNode and receives some epoch change verify along with a latest
    /// ledger info inside the `UpdateToLatestLedgerResponse`. This function
    /// verifies the change verify and ratchets the trusted state version forward
    /// if the response successfully moves us into a new epoch or a new latest
    /// ledger info within our current epoch.
    ///
    /// + If there was a validation error, e.g., the epoch change verify was
    ...
    ///
    /// + If there is a new epoch and the server provides a correct verify, we
